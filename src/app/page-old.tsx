...
export default function HomePage() {
  ...
  // Scroll progress mapping - MUCH shorter scroll distance
  const scrollProgress = useTransform(scrollY, [0, 3000], [0, 1])
  
  // Logo (1) - visible on page load, disappears quickly
  const logoScale = useTransform(scrollProgress, [0, 0.15, 0.3], [1, 1, 0])
  const logoOpacity = useTransform(scrollProgress, [0, 0.1, 0.3], [1, 1, 0])
  
  // ABEL appearance - writes on the 1 after page load  
  const labelOpacity = useTransform(scrollProgress, [0, 0.05, 0.25], [0, 1, 0])
  
  // Content appears much sooner and stays longer
  const contentOpacity = useTransform(scrollProgress, [0.15, 0.25, 0.9, 1], [0, 1, 1, 0])
  const contentScale = useTransform(scrollProgress, [0.15, 0.3], [0.8, 1])
  
  // Different sections appear at different scroll points
  const section1Opacity = useTransform(scrollProgress, [0.2, 0.3, 0.6, 0.7], [0, 1, 1, 0])
  const section2Opacity = useTransform(scrollProgress, [0.3, 0.4, 0.7, 0.8], [0, 1, 1, 0])
  const section3Opacity = useTransform(scrollProgress, [0.4, 0.5, 0.8, 0.9], [0, 1, 1, 0])
  const section4Opacity = useTransform(scrollProgress, [0.5, 0.6, 0.9, 1], [0, 1, 1, 0])
  ...
  return (
    <div ref={containerRef} className="bg-black text-white overflow-hidden">
      ...
      {/* Much shorter spacer for scroll */}
